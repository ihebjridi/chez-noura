...
function Dialog({ children, transition, isOpen: controlledIsOpen, onOpenChange }: DialogProps) {
...
function DialogTrigger({
  ...
  const { setIsOpen, isOpen, uniqueId, triggerRef: contextTriggerRef } = useDialog();
  const ref = externalTriggerRef || contextTriggerRef;
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            data-lenis-prevent
            className={cn(
              'fixed inset-0 h-full z-50 w-full backdrop-blur-xl bg-[radial-gradient(125%_125%_at_50%_10%,#ffffff_40%,#243aff_100%)]',
              overlayClassName
            )}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
